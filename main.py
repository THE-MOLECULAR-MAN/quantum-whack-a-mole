...
def classical_smash(model, hole):
    ...
    return (measured_hole == [0, 0] and hole == 1) or \
        (measured_hole == [0, 1] and hole == 2) or \
        (measured_hole == [1, 0] and hole == 3) or \
        (measured_hole == [1, 1] and hole == 4)
...
def quantum_smash(model, hole):
    ...
    if gate_details[0] == 'r' or gate_details[0] == 'R':
        model.add_r_gate(math.radians(float(gate_details[1])),
                         int(gate_details[2]))
